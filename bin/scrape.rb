...
def scrape (id, page)
  ...
  list.each do |li|
    datetime = li.css('.updatetime').text.strip + '+09'
    ...
    if $conn.exec('SELECT url FROM item WHERE url = $1', [ url ]).count == 0
      $conn.exec(
        'INSERT INTO item (url, member_id, title, count, datetime) VALUES ($1::varchar, $2::integer, $3::varchar, $4::int, $5::timestamp with time zone)',
        [ url, id, title, count, datetime ]
      )
      $log.debug("insert - #{url}: #{count}")
    else
      $conn.exec(
        'UPDATE item SET count = $1::integer, datetime = $2::timestamp with time zone WHERE url = $3::varchar',
        [ count, datetime, url ]
      )
